...
**The Question:** For decades, a common belief in the Intermountain West real estate market has been that the announcement of a Latter-day Saint temple guarantees a significant spike in neighboring property values. Some [real estate agents](https://universe.byu.edu/2019/10/08/new-temples-bring-excitement-and-higher-housing-prices/), [journalists](https://cowboystatedaily.com/2023/10/06/lds-temples-may-be-opposed-for-many-reasons-but-they-boost-property-values/), and [developers](https://bhroberts.org/records/0aH439-stEmCb/paul_gilbert_tells_arizona_republic_he_expects_the_temple_to_benefit_property_values) cite a "Temple Boost" or a "Temple Effect" of 10–20%, pitching temple sites as sure-fire investment opportunities. This report investigates whether that premium is a causal result of the temple itself, or a statistical illusion.
...
When The Church of Jesus Christ of Latter-day Saints announces a new temple, there can be many local reactions.

On one side, real estate agents and faithful investors see dollar signs. They circulate the folklore of the "Temple Boost"—a belief that the arrival of a temple acts as a divine anchor, guaranteeing that neighboring home values will skyrocket by 10% to 20% or even double. To them, the temple is the ultimate amenity.
...
To the Critics: There is no evidence that temples destroy property values at least at the community/neighborhood level. While immediate neighbors may experience localized nuisance effects, our data confirms these are not severe enough (if they do exist) to drag down the broader zip code.
...
The wave is *real* — prices in these areas do go up — but the temple is just another passenger on the ride, not the engine driving it.
...
The Defense: Our event-study plots (Figure 5 in the technical appendix) allow us to visually inspect the pre-announcement period. We found no statistically significant 'pre-loading' of prices in the 12 months prior to announcement, suggesting that information leakage was not a primary driver of price appreciation in these markets.
...
Our analysis of Census records for 80 temple sites compared to the national averages establishes a clear "Winner’s Profile." The Church systematically selects neighborhoods that are already winning economically years before the announcement is made.
...
In the "Mission Field" (e.g., Boston/New York): It could be viewed as a "disamenity." Residents may view it not as a holy site, but as a commercial-scale structure that brings traffic and lighting to residential zones. Here, critics argue, the impact should be negative.
...
Economically, this directional lift could reflect the difference between a Passive Amenity and an Active Necessity.
...
The positive directional signal in high-LDS areas could suggest that members do value proximity, acting as a "tie-breaker" when choosing between similar homes. However, the saturation of temples in these regions may ensure that this preference never hardens into a measurable financial premium. The amenity may be too abundant to be expensive.
...
Our findings offer a likely resolution to the "Chicken or Egg" paradox. While homes in temple zip codes undeniably appreciate faster than the national average, the data indicates that the primary driver of this growth is not caused by the temple. Rather, the temple is systematically placed in neighborhoods that are already experiencing strong growth trajectories driven by fundamental economic forces. The "Temple Premium," therefore, is likely an artifact of Site Selection Bias. The Church’s selection criteria favor zip codes with high educational attainment, established household wealth, and robust population characteristics—the very factors that urban economists identify as the primary drivers of appreciation.

Crucially, this "Null Result" holds across all cultural contexts from what we can detect. Whether in the high-density "Zion" markets of Utah—where demand for the amenity is highest—or in the low-density "Mission Field" markets where resistance is more common, the causal impact of the structure itself remains statistically indistinguishable from zero. Though as temples continue to be built, a more modest price increase or decrease may be detectable in higher powered future studies.
...
To estimate the causal impact of temple announcements on housing values, we utilize the Callaway & Sant'Anna (2021) estimator for staggered treatment adoption. This framework addresses two critical failures of standard Two-Way Fixed Effects (TWFE) models in this context: (1) it allows for treatment effects to vary over time (heterogeneity), and (2) it avoids the "negative weighting bias" where early-treated units act as bad controls for late-treated units (Goodman-Bacon, 2021).

1. **Identification Strategy: Group-Time Average Treatment Effects** We estimate the group-time average treatment effect, denoted as $ATT(g,t)$, which measures the average treatment effect for the cohort of zip codes first treated in period $g$ at calendar time $t$.$$ATT(g,t) = \mathbb{E}[Y_t - Y_{g-1} \mid G_g = 1] - \mathbb{E}[Y_t - Y_{g-1} \mid C_t = 1]$$ Where: 
* $Y_t$ is the outcome (log Zillow Home Value Index) at time $t$
* $G_g = 1$ denotes the group of zip codes where a temple was announced in period $g$
* $C_t = 1$ denotes the "not-yet-treated" control group. This includes both units that are never treated in the sample and units that are treated at some future period $g > t$
* $t$ is the current time period, and $g-1$ is the reference period immediately prior to treatment.

2. **Conditional Parallel Trends Assumption** Given that temple site selection is non-random (often targeting areas with specific growth or demographic profiles), the standard unconditional parallel trends assumption is unlikely to hold. We therefore invoke the Conditional Parallel Trends assumption. We assume that, conditional on a vector of baseline covariates $X$ (measured at the 2000 Census baseline), the average potential outcomes for the treated and control groups would have followed parallel paths in the absence of treatment:$$\mathbb{E}[Y_t(0) - Y_{g-1}(0) \mid X, G_g = 1] = \mathbb{E}[Y_t(0) - Y_{g-1}(0) \mid X, C_t = 1]$$Our vector $X$ includes baseline log income, population density, homeownership rates, share of households with children, and education levels, alongside State Fixed Effects to account for unobserved regional heterogeneity. Additionally, in the overall model, we included a county level share of population that is LDS.

3. **Estimation Method: Outcome Regression** To estimate $ATT(g,t)$ under this assumption, we employ the Outcome Regression (OR) approach (via the did package in R). We fit a linear regression model of the change in outcomes $\Delta Y_t = Y_t - Y_{g-1}$ for the control group as a function of covariates $X$:$$\Delta Y_t = \alpha + X'\beta + \epsilon$$We then project these estimated counterfactual trends onto the treated group to recover the causal effect:$$\widehat{ATT}(g,t) = \frac{1}{N_g} \sum_{i \in G_g} \left( (Y_{i,t} - Y_{i,g-1}) - \hat{m}_{g,t}(X_i) \right)$$Where $\hat{m}_{g,t}(X_i)$ is the predicted change in the outcome for unit $i$ based on the control group's conditional evolution. For statistical inference, we utilize a multiplier bootstrap procedure with standard errors clustered at the county level to account for serial correlation and spatial dependence within local housing markets.

4. **Aggregation (Event Study Specification)** To examine the dynamic evolution of treatment effects and test for pre-trends (as shown in Figure 5), we aggregate the individual group-time average treatment effects according to "event time" $e = t - g$ (the quarters elapsed since announcement). We estimate the Event Study Average Treatment Effect $ATT_{es}(e)$:$$\widehat{ATT}_{es}(e) = \sum_{g} \sum_{t} \mathbb{1}(t - g = e) \cdot w_{g,t} \cdot \widehat{ATT}(g,t)$$Where $\mathbb{1}(t - g = e)$ is an indicator function that selects all group-time pairs that are exactly $e$ periods away from treatment, and $w_{g,t}$ are weights based on group size. This allows us to estimate the causal effect for each quarter before and after the announcement, providing a direct test of the parallel trends assumption (where $e < 0$) and the treatment effect trajectory (where $e \ge 0$).
...
#| warning: false
#| message: false
#| tbl-cap: "List of 42 Temple Announcements Included in Analysis"
#| label: tbl-temples
...
temple_list <- read.csv('./temples.csv') |> 
  mutate(
    Announced = mdy(Announced),
    Zipcode = stringr::str_pad(Zipcode, width = 5, side = "left", pad = "0")
    ) |> 
  filter(year(Announced)>=2011) |> 
  filter(Zipcode != "84627") |> 
  select(Temple, Announced, Zipcode)

temple_list %>%
  # 1. Clean up columns for display
  select(Temple, Announced, Zipcode) %>%
  arrange(desc(Announced)) %>%
  
  # 2. Rename columns to look professional
  rename(
    "Temple Name" = Temple,
  ...
  # 3. Create the table
  # 'longtable = TRUE' allows it to span multiple pages if needed
  ...
  kable_styling(
    ...
    font_size = 10, # Shrink slightly to fit nicely
    full_width = FALSE
  )

```

Originally, I had planned to include about 80 temples announced since 2000; however, due to the sparsity of zillow data before 2010, I decided to filter to temples announced 2011 and after (43 temples) giving at least a full year of complete pretrend zillow data. There was one temple not included in the Zillow data likely because of its rural location. This temple is Ephraim Utah in zip code 84627. All other temples announced in the US since 2011 are included (42 temples). I also filtered specifically to those that had the ground broken by December of 2025 -- meaning for every temple in this list, there is either a completed temple in the zip code or a current temple construction site in the most recent Zillow data.
...
Code Repository: The R scripts and Quarto source files are hosted on GitHub at: [click here!](https://github.com/acbass49/temple_impact_report_2026) Please see the repository for the replication guide and analysis decisions.
...
Data Access: Due to licensing restrictions regarding granular real estate data, the raw housing dataset cannot be publicly redistributed. In the readme in the repository linked above, I have information on where I downloaded each dataset.
