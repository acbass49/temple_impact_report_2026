...
set.seed(123)
out_quarterly <- did::att_gt(
    yname = "log_price",
    tname = "quarter_index",      # Time is now our continuous quarter counter
    idname = "id_numeric",
    gname = "treat_quarter_index",# Group is the quarter they were treated
    xformla = ~1,                 # Explicitly saying "No Controls"
    data = quaterly_model_data,
    control_group = "notyettreated",
    clustervars = "cluster_id",
    allow_unbalanced_panel = TRUE,
    bstrap = TRUE,
    pl = TRUE, # Turn on parallel computing
    cores = 2
)

es_quarterly <- aggte(out_quarterly, type = "dynamic", na.rm = TRUE)
...
out_quarterly_controls <- did::att_gt(
    ...
    est_method = "reg", # using regression adjustment instead of PSM because of missingness
    bstrap = TRUE,
    pl = TRUE, # Turn on parallel computing
    cores = 2
)

# 3. Aggregate to Event Time
# --------------------------
...
out_quarterly_high <- did::att_gt(
    ...
    # matching formula
    xformla = ~base_log_inc + base_log_pop + base_pct_own + base_pct_kids + base_pct_educ + base_year_built + state,
    
    data = quaterly_model_data_high,
    control_group = "notyettreated",
    clustervars = "cluster_id",
    allow_unbalanced_panel = TRUE,
...
es_quarterly_high <- aggte(out_quarterly_high, type = "dynamic", na.rm = TRUE)
...
set.seed(123)
out_quarterly_very_high <- did::att_gt(
    yname = "log_price",
    tname = "quarter_index",      # Time is now our continuous quarter counter
    idname = "id_numeric",
    gname = "treat_quarter_index",# Group is the quarter they were treated
    ...
    # matching formula
    xformla = ~base_log_inc + base_log_pop + base_pct_own + base_pct_kids + base_pct_educ + base_year_built,
    
    data = quaterly_model_data_very_high,
    control_group = "notyettreated",
    clustervars = "cluster_id",
    allow_unbalanced_panel = TRUE,
